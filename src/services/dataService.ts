  ...
  importJSON(data: any[]) {
    const newRecords: CarRecord[] = data.map(item => {
      return {
        ...
        sectorName: (item['NOME_SETOR'] || item['SETOR_NOME'] || item['DESC_SETOR'] || item['SETOR_DESC'] || item['DESCRICAO'] || item['DESC'] || item['SETOR'] || '').toString(),
  ...
  importCSV(csvText: string) {
    ...
    const newRecords: CarRecord[] = lines.slice(1).filter(line => line.trim()).map(line => {
      ...
      const mapping: Record<string, keyof CarRecord> = {
        ...
        'DESC_SETOR': 'sectorName',
        'SETOR_DESC': 'sectorName',
        'DESCRICAO': 'sectorName',
        'DESC': 'sectorName',
        'LOC_FISICA': 'location',
